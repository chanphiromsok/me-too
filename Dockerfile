...
RUN apt-get update \
  && apt-get install -y --no-install-recommends libstdc++6 libsctp1 openssl libncurses6 locales ca-certificates curl tini \
  && rm -rf /var/lib/apt/lists/*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/me ./
