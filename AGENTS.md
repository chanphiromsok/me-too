...
<!-- ash_state_machine-start -->
## AshStateMachine guidelines

- Add `AshStateMachine` to a resource's extensions before using the `state_machine` DSL.
- Define `initial_states` and `default_initial_state` explicitly.
- Request state changes inside actions with `change transition_state(:target_state)`.
- Define a matching transition for every state-changing action. A requested transition fails when its action, source state, and target state do not match the configured transitions.
<!-- ash_state_machine-end -->
